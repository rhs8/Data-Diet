...
function renderFoodRow(selectedId = 'vegetables', servings = 1) {
  const opts = FOODS.map(f => `<option value="${f.id}" ${f.id === selectedId ? 'selected' : ''}>${escapeHtml(f.name)}</option>`).join('');
  return `
...
function renderAccountDashboard() {
  ...
  if (!user) {
    mount.innerHTML = `
      <details class="methodology-blurb account-methodology">
        ...
        <p>Intensities are per kg of product (from <code>data.js</code>). A meal line multiplies by <strong>servings</strong>. Adding different metrics (GHG, land, …) mixes units. Treat combined totals as an <strong>exploratory index</strong>, not one physical quantity. For a single interpretable measure, use <strong>GHG only</strong> (kg CO₂) in trends or the Breakdown tab toggle.</p>
      </details>
  ...
  mount.innerHTML = `
    <details class="methodology-blurb account-methodology">
      <summary>How numbers work</summary>
      <p>Each meal line is <strong>per-kg intensity × servings</strong>. Summing every metric into one number mixes units (kg CO₂, m², …). Use that <strong>index</strong> for relative trends. For one clear unit, turn on <strong>GHG only</strong> below in Your trends, or use the Breakdown tab checkbox.</p>
    </details>
