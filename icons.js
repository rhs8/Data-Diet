// ═══════════════════════════════════════════════════════════════
// icons.js — inline SVG icons (foods + nav)
// Food glyphs: Lucide Icons v0.460.0 (ISC) — one icon per dataset food id
// https://lucide.dev
// ═══════════════════════════════════════════════════════════════

const LUCIDE_FOOD_PATHS = {
  ...
  _default: '<circle cx="12" cy="12" r="9"/><path d="M8 12h8M12 8v8"/>',
};

const FOOD_ICON_LUCIDE_KEY = {
  'apples': 'apple',
  'bananas': 'banana',
  'barley-beer': 'beer',
  'beef-beef-herd': 'beef',
  'beef-dairy-herd': 'beef',
  'beet-sugar': 'candy',
  'berries-grapes': 'grape',
  'brassicas': 'sprout',
  'cane-sugar': 'candy',
  'cassava': 'carrot',
  'cheese': 'pizza',
  ...
  'rapeseed-oil': 'droplet',
  'rice': 'soup',
  'root-vegetables': 'carrot',
  'shrimps-farmed': 'shell',
  'soybean-oil': 'droplet',
  'soymilk': 'milk',
  'sunflower-oil': 'sprout',
  'tofu': 'box',
  'tomatoes': 'cherry',
  'wheat-rye-bread': 'sandwich',
  'wine': 'wine',
};

function foodIconPathsFor(foodId) {
  const k = FOOD_ICON_LUCIDE_KEY[foodId];
  if (k && LUCIDE_FOOD_PATHS[k]) return LUCIDE_FOOD_PATHS[k];
  return LUCIDE_FOOD_PATHS._default;
}
...
function foodIconHtml(foodId, px = 20, className = 'icon-food') {
  const paths = foodIconPathsFor(foodId);
  if (!paths) return '';
